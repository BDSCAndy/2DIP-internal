...
def view_orderr():
    ...
    if sum_1>0:
        for i in order:
            pr_ice=pr_ice+i
        messagebox.showinfo('Order',f"{pr_ice}----------------------------\nTotal price is: ${sum_1:.2f}")
        return
    else:
        messagebox.showerror('Checkout',"Please make an order.")

def check_out():# Functions for frames 2-6, This function displays the order and total price and logs the user out and takes the back to the age.
    global sum_1
    view_orderr()
    if sum_1>0:
        order.clear()
