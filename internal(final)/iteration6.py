...
def frame0():# This function asks for users age
    ...
    def age():
        try:
            ...
            messagebox.showinfo('age',"Please enter a number")# Value error incase user enters a number. 

    frame_0=Frame(root,bg="#5a183a")# This is a red background colour
    frame_0.grid(row=0,column=0,sticky='nsew') 
    
    image0 = Image.open("image0.png")# Inserting image.
    ...
    age_lbl=Label(frame_0,text="Please enter your age:",background="#5a183a",foreground="White")# Seting label background colour the same as the frame background colour. 
...
def frame_1():# This function allows the user to either add account, login or remove password. 
    ...
    def confirm():
        ...
        if not ac_account or not password:#If the user has not entered  anything.
                messagebox.showerror('Incorrect password', 'Please make an entry')
                found=True
                return# Ends the function
        else:
            return
    def confirm_login():# This function determines if userinput is==to the txt file username and password.
        global  found2
        found2=False
...
def frame_2():# Menu
    ...
    image2 = Image.open("image2.jpg")# Drinks image button
    ...
    image3 = Image.open("image3.jpg")#Lunch image button
    ...
    image4 = Image.open("image4.jpg")#Snacks image button
    ...
    image5 = Image.open("image5.jpg")#Frozen image button
...
def add_all():# This function clears the order and total price
     global sum_1
     order.clear()
     sum_1=0
...
def check_out():# This function shows the total price and order and logs the user out
    ...
    if sum_1>0:
        ...
        messagebox.showerror('Checkout',"Please make an order.")
def ad_ord():# This function adds the orde to the total 
        found=False
        global sum_1,order
...
def remove_order():# This function removes  the specific selected order.
        found=False
        global sum_1,order
        try:
            for var, name, price, spinbox in zip(track_var, category_names, category_price, track_spinbox):
...
def upload():#This function displays what the user has ordered in the Text widget.
        global box4
        box4.delete(1.0,END)
        for i in order:# Using for loop to insert it. 
            box4.insert(END,f"{i}")
            box4.insert(END,"--------------------------------")
def frame_3():# Drinks menu
    global track_var,category_names,category_price,track_spinbox,box4,spinbox,spinbox,check
    track_var=[]# Stores the checkboxes
    track_spinbox=[]#Stores the spinboxes
    ...
    box4 = scrolledtext.ScrolledText(frame3,width=32,height=10)# Adding buttons and scroll text widget.
    box4.grid(column=0,row=32,sticky=W,columnspan=2)
    
    back2=Button(frame3,text="Snacks",command=frame_5,width=10)# These buttons are for quick navigation
...
def frame_4():#Lunch menu
    ...
    box4 = scrolledtext.ScrolledText(frame4,width=32,height=5)# adding scrolltext and buttons
    box4.grid(column=0,row=32,sticky=W,columnspan=2)
...
def frame_5():#Snacks menu
    ...
    box4 = scrolledtext.ScrolledText(frame5,width=32,height=13)# adding more scrolltext and buttons
...
def frame_6():#Frozen menu
    ...
    box4 = scrolledtext.ScrolledText(frame6,width=32,height=10)# Adding more scroll text and buttons
    box4.grid(column=0,row=32,sticky=W,columnspan=2)
    
    back2=Button(frame6,text="Drinks",command=frame_3,width=10)# These buttons are for quick navigation
    back2.grid(column=0,row=33,sticky=W)
